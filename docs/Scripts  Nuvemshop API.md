...
## Important[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#important "Direct link to Important")
...
-   The script is injected in the merchant storefront and a store (id) parameter is sent along with it
    ...
    ```
    <script type="text/javascript" src="https://apps-scripts.tiendanube.com/app-handle/script-handle/2.js?versionId=c1285a470f0fc8f14f54851c5d8eb32f&store=1234"></script>
    ```
...
## Script code[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#script-code "Direct link to Script code")
...
### Considerations[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#considerations "Direct link to Considerations")

#### Clojure[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#clojure "Direct link to Clojure")
...
#### AJAX[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#ajax "Direct link to AJAX")
...
#### JQuery[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#jquery "Direct link to JQuery")

In case jQuery is necessary, it should be accessed by the `useJquery` method. Some stores already have jQuery in their themes, but it's not necessarily the latest version. Some store's themes includes their own version of JQuery, which might vary. If the store theme doesn't use it, then a default recent version will be provided (3.6)
...
### Variables[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#variables "Direct link to Variables")

A Javascript object (called `LS`) with some common variables is available in the Script context. The object contains general Tiendanube's domain information that might be relevant for the script implementation.

Each storefront page has its own `LS` object definition

#### Store Page[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#store-page "Direct link to Store Page")
...
#### Product Page[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#product-page "Direct link to Product Page")
...
#### Category Page[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#category-page "Direct link to Category Page")

```
LS.category = {    id : /* Category's id */,    name : /* Category's name */};
```

#### Checkout Page[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#checkout-page "Direct link to Checkout Page")
...
> Note: You cannot access this variable from the [JavaScript file](https://tiendanube.github.io/api-documentation/resources/checkout#payment-options-javascript-interface) where developers can create their own Payment Options.

#### Thank you page[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#thank-you-page "Direct link to Thank you page")
...
## Scripts Management[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#scripts-management "Direct link to Scripts Management")
...
That being said, a Script can have multiple versions in its lifecycle. Let's say that the goal of a script is to provide a way for Merchant's clients to select previous lost Cart, the starting version of the script might show a dialog with past carts, and a future version of the script might integrate this past carts in the page's interface. There must be a way for the user to select a past cart (`WHAT`) and the App script will provide a way to do so, and the way it does that might change overtime in different versions.

### Creating a Script[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#creating-a-script "Direct link to Creating a Script")
...
### Script form properties[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#script-form-properties "Direct link to Script form properties")
...
#### Important Rules[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#important-rules "Direct link to Important Rules")

-   Some fields have a deep impact on how the Script will behave. So lets deep dive into it.

##### Event field[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#event-field "Direct link to Event field")
...
###### onfirstinteraction Event[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#onfirstinteraction-event "Direct link to onfirstinteraction Event")
...
###### onload event[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#onload-event "Direct link to onload event")
...
In this email you must send the functionality of the script and why it should be executed `onload`. Please include the `APP_ID` and the `APP_NAME` in the email title. If not approved, the script will be created with the **onfirstinteraction** event.

##### Development mode[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#development-mode "Direct link to Development mode")
...
###### Development Url[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#development-url "Direct link to Development Url")
...
##### Development query parameters (Manual Scripts)[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#development-query-parameters-manual-scripts "Direct link to Development query parameters (Manual Scripts)")
...
##### Script file[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#script-file "Direct link to Script file")
...
##### Auto Installed[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#auto-installed "Direct link to Auto Installed")
...
###### Not auto installed script flow example[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#not-auto-installed-script-flow-example "Direct link to Not auto installed script flow example")

Imagine a partner that has an app of id `<APP_ID>` and a script id `<SCRIPT_ID>` associated with it. The script behavior changes based on the Partner's system business rule. Let's say that the partner wants to show a Dialog congratulating the 1000th installation of its app on the Merchant's Storefront. The dialog will be shown if the script is loaded in the storefront given a parameter `showDialog` true.

Let's say that the merchant of id `<STORE_ID>` is the 1000th installer of our Partner's app. Once the merchant installs the app, the partner will receive a webhook informing the app with id `<APP_ID>` was installed in the store `<STORE_ID>`. The partner's system will then check that this is the 1000th installation, and then it's necessary to send a request to:

```
curl --location 'https://api.tiendanube.com/2025-03/<STORE_ID>/scripts' \--header 'Authentication: bearer <ACCESS_TOKEN>' \--header 'User-Agent: PartnerAppName partner@email.com' \--header 'Content-Type: application/json' \--data '{    "script_id": <SCRIPT_ID>,    "query_params": "{\"showDialog\": true}"}'
```

From this moment on, the script will be loaded in this specific merchant storefront in the following format:

```
<script src="https://apps-scripts.tiendanube.com/app-handle/script-handle/2.js?versionId=c1285a470f0fc8f14f54851c5d8eb32f&store=1234&showDialog=true" />
```
...
```
curl --location 'https://api.tiendanube.com/2025-03/<ANOTHER_STORE_ID>/scripts' \--header 'Authentication: bearer <ACCESS_TOKEN>' \--header 'User-Agent: PartnerAppName partner@email.com' \--header 'Content-Type: application/json' \--data '{    "script_id": <SCRIPT_ID>,    "query_params": "{}"}'
```

-   The script will then be loaded in the storefront:

```
<script src="https://apps-scripts.tiendanube.com/app-handle/script-handle/2.js?versionId=c1285a470f0fc8f14f54851c5d8eb32f&store=1234" />
```
...
### Versioning and Deploy[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#versioning-and-deploy "Direct link to Versioning and Deploy")
...
#### Adding version[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#adding-version "Direct link to Adding version")
...
#### Deploy testing[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#deploy-testing "Direct link to Deploy testing")
...
#### Deploy[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#deploy "Direct link to Deploy")
...
#### Rollback[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#rollback "Direct link to Rollback")
...
## Scripts Public API Endpoints[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#scripts-public-api-endpoints "Direct link to Scripts Public API Endpoints")
...
### GET /scripts[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-scripts "Direct link to GET /scripts")
...
#### Response[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#response "Direct link to Response")
...
### GET /scripts/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-scriptsid "Direct link to GET /scripts/{id}")

#### Response[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#response-1 "Direct link to Response")
...
### POST /scripts[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-scripts "Direct link to POST /scripts")
...
#### Request Body[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#request-body "Direct link to Request Body")

```
{  "script_id": 12345,  "query_params": "{\"paramA\": 2}"}
```

#### Response[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#response-2 "Direct link to Response")
...
### PUT /scripts/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#put-scriptsid "Direct link to PUT /scripts/{id}")

-   Modifies an existing script-store association (parameters)
-   Only available for scripts defined as "Non-autoinstallable"

#### Request Body[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#request-body-1 "Direct link to Request Body")

```
{  "script_id": 12345,  "query_params": "{\"paramA\": 3}"}
```

#### Response[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#response-3 "Direct link to Response")
...
### DELETE /scripts/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-scriptsid "Direct link to DELETE /scripts/{id}")

-   Removes script association with store
    -   Script stops being loaded in informed store
-   Only available for scripts defined as "Non-autoinstallable"

#### Response[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#response-4 "Direct link to Response")
