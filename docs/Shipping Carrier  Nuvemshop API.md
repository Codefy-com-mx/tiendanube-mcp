...
> **_Note:_** To create a Shipping App, you first need to create an App in the Partners Portal and fill out the following form: [https://forms.gle/oqP1BrtwMzNb7xCM9](https://forms.gle/oqP1BrtwMzNb7xCM9). This step is required for our Platform Team to grant you access to the shipping endpoints.

> **_Note:_** If you have any questions, feel free to contact us at: [partners@nuvemshop.com.br](mailto:partners@nuvemshop.com.br) / [partners@tiendanube.com](mailto:partners@tiendanube.com).

## Shipping Carrier Properties[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#shipping-carrier-properties "Direct link to Shipping Carrier Properties")
...
| callback\_url | The URL endpoint that we need to retrieve shipping rates. **Must be HTTPS.** |
| types | The supported shipping types, can be one or both _ship_ or _pickup_. |
| active | Whether this Shipping Carrier is active. |
| created\_at | Date when the Shipping Carrier was created in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). |
| updated\_at | Date when the Shipping Carrier was last updated in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). |

## Shipping Carrier Options[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#shipping-carrier-options "Direct link to Shipping Carrier Options")
...
## Shipping Carrier Options Properties[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#shipping-carrier-options-properties "Direct link to Shipping Carrier Options Properties")
...
| additional\_days | The additional days configurable value that will be added to the option's estimated delivery time. |
| additional\_cost | The additional cost configurable value that will be added to the option's consumer price. |
| allow\_free\_shipping | The configurable free shipping eligible parameter that specifies that an option allows free shipping. |
| active | Whether this Shipping Carrier Option is active. |
| created\_at | Date when the Shipping Carrier Option was created in [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601). |
| updated\_at | Date when the Shipping Carrier Option was last updated in [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601). |

## Providing rates to our merchants[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#providing-rates-to-our-merchants "Direct link to Providing rates to our merchants")
...
> _**Important:**_ you have to post all your available rates, our API will filter by carrier options active, and apply the settings made by users over the rates (additional days and additional cost).

> _**Note:**_ Additional information about `price` and `price_merchant` related to `free shipping` can be found in this [guide](https://tiendanube.github.io/api-documentation/guides/shipping-provider#shipping-rates).

> _**Note:**_ Check out [this video](https://www.youtube.com/watch?v=wGpyOO1pBkw) where we show how price and price\_merchant rules work.

> _**Note:**_ It's possible to send the same `code` for more than one **_pickup_ option** in the rates. For **_ship_ options**, the `code` cannot be repeated. If two options with the same code are sent, we will only process the first one.
...
| type | **(required)** The rate's type: _ship_ if it will be deliverd to a buyer's address or _pickup_ if it will be picked up by the buyer. |
| address | **(required)** _(pickup only)_ Where the pickup point is located. |
| hours | **(required)** _(pickup only)_ The location open hours providing: `day` (0: sunday, 6: saturday), `start` and `end` on `HHMM` format. |
| price\_merchant | The rate's price that will be payed by the merchant. It may be different than `price` due to items with free shipping. Defaults to `price`. Check out [this video](https://www.youtube.com/watch?v=wGpyOO1pBkw) for more info. |
| min\_delivery\_date | The earliest estimated delivery date in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). Defaults to `null`. |
| max\_delivery\_date | The latest estimated delivery date in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). Defaults to `null`. |
| id\_required | Whether the customer must provide an identification number at checkout. Defaults to `false`. |
| phone\_required | Whether the customer must provide a phone number at checkout. Defaults to `false`. |
| accepts\_cod | Whether the customer is allowed to pay for this rate with a Cash on Delivery (COD) payment method (defaults to `true`). |
...
### Example requests for shipping rates[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#example-requests-for-shipping-rates "Direct link to Example requests for shipping rates")

#### POST /your\_callback\_url[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-your_callback_url "Direct link to POST /your_callback_url")
...
## Server-side caching of requests[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#server-side-caching-of-requests "Direct link to Server-side caching of requests")
...
### Cache Key Composition[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#cache-key-composition "Direct link to Cache Key Composition")

The cache key is built as an MD5 hash of the following components:

| Component | Fields |
| --- | --- |
| carrier\_id | Shipping carrier / app ID |
| origin | name, location\_id, address, number, floor, locality, city, province, country, postal\_code, phone |
| destination | name, address, number, floor, locality, city, province, country, postal\_code, phone |
| items\_hash | MD5 of each item's `{id}:{quantity}:{grams}:{dimensions.width}:{dimensions.height}:{dimensions.depth}` |
...
### Cache TTL (Time to Live)[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#cache-ttl-time-to-live "Direct link to Cache TTL (Time to Live)")
...
## Circuit Breaker for Unstable Shipping Carrier[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#circuit-breaker-for-unstable-shipping-carrier "Direct link to Circuit Breaker for Unstable Shipping Carrier")
...
### HTTP Response Handling and Fallback[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#http-response-handling-and-fallback "Direct link to HTTP Response Handling and Fallback")
...
> **Recommendation:** If your application needs to refuse a shipment for a known business reason (e.g., the destination is outside your coverage area), always return a **4xx** status code instead of a **5xx**. A `4xx` response is counted as a _success_ for circuit breaker purposes, so it will not degrade your carrier’s health metrics, while still causing the platform to display fallback shipping options to the buyer.

### Circuit Breaker Parameters[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#circuit-breaker-parameters "Direct link to Circuit Breaker Parameters")
...
### POST /shipping\_carriers[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-shipping_carriers "Direct link to POST /shipping_carriers")
...
| callback\_url | **(required)** The URL endpoint that we need to retrieve shipping rates when the buyer wants the items ship to an address. |
...
#### POST /shipping\_carriers[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-shipping_carriers-1 "Direct link to POST /shipping_carriers")
...
### GET /shipping\_carriers[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carriers "Direct link to GET /shipping_carriers")

Receive a list of all Shipping Carriers.

#### GET /shipping\_carriers[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carriers-1 "Direct link to GET /shipping_carriers")
...
### GET /shipping\_carriers/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carriersid "Direct link to GET /shipping_carriers/{id}")

Receive a single Shipping Carrier

#### GET /shipping\_carrier/1[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carrier1 "Direct link to GET /shipping_carrier/1")
...
### PUT /shipping\_carriers/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#put-shipping_carriersid "Direct link to PUT /shipping_carriers/{id}")

Modify an existing Shipping Carrier

#### PUT /shipping\_carriers/123[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#put-shipping_carriers123 "Direct link to PUT /shipping_carriers/123")
...
### DELETE /shipping\_carriers/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-shipping_carriersid "Direct link to DELETE /shipping_carriers/{id}")

Remove a Shipping Carrier

#### DELETE /shipping\_carriers/123[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-shipping_carriers123 "Direct link to DELETE /shipping_carriers/123")

`HTTP/1.1 200 OK`

### POST /shipping\_carriers/{carrier\_id}/options[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-shipping_carrierscarrier_idoptions "Direct link to POST /shipping_carriers/{carrier_id}/options")
...
| additional\_days | The additional days configurable value that will be added to the option's estimated delivery time. | No |
| additional\_cost | The additional cost configurable value that will be added to the option's consumer price. | No |
| allow\_free\_shipping | The configurable free shipping eligible parameter that specifies that an option allows free shipping. | No |
| active | The avaiability status of the Shipping Carrier Option. Defaults to **true** | No |

#### POST /shipping\_carriers/1234/options[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-shipping_carriers1234options "Direct link to POST /shipping_carriers/1234/options")
...
### GET /shipping\_carriers/{carrier\_id}/options[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carrierscarrier_idoptions "Direct link to GET /shipping_carriers/{carrier_id}/options")

Receive a list of all Shipping Carriers Options.

#### GET /shipping\_carriers/1234/options[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carriers1234options "Direct link to GET /shipping_carriers/1234/options")
...
### GET /shipping\_carriers/{carrier\_id}/options/{option\_id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carrierscarrier_idoptionsoption_id "Direct link to GET /shipping_carriers/{carrier_id}/options/{option_id}")

Receive a single Shipping Carrier Option.

#### GET /shipping\_carrier/1234/options/123[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-shipping_carrier1234options123 "Direct link to GET /shipping_carrier/1234/options/123")
...
### PUT /shipping\_carriers/{carrier\_id}/options/{option\_id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#put-shipping_carrierscarrier_idoptionsoption_id "Direct link to PUT /shipping_carriers/{carrier_id}/options/{option_id}")

Modify an existing Shipping Carrier Option.

#### PUT /shipping\_carriers/1234/options/123[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#put-shipping_carriers1234options123 "Direct link to PUT /shipping_carriers/1234/options/123")
...
### DELETE /shipping\_carriers/{carrier\_id}/options/{option\_id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-shipping_carrierscarrier_idoptionsoption_id "Direct link to DELETE /shipping_carriers/{carrier_id}/options/{option_id}")

Remove a Shipping Carrier Option

#### DELETE /shipping\_carriers/1234/options/123[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-shipping_carriers1234options123 "Direct link to DELETE /shipping_carriers/1234/options/123")

`HTTP/1.1 200 OK`

## Fulfillment Events[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#fulfillment-events "Direct link to Fulfillment Events")
...
> 📢 **Important** 📢
> 
> A new [Fulfillment Events API](https://tiendanube.github.io/api-documentation/guides/multi-inventory#changes-to-fulfillment-events) with support for multi inventory is currently being rolled out to every merchant.
> 
...
### GET /orders/{order\_id}/fulfillments[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-ordersorder_idfulfillments "Direct link to GET /orders/{order_id}/fulfillments")
...
### GET /orders/{order\_id}/fulfillments/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-ordersorder_idfulfillmentsid "Direct link to GET /orders/{order_id}/fulfillments/{id}")

Receive a single Fulfillment Event

#### GET /orders/123/fulfillments/1[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#get-orders123fulfillments1 "Direct link to GET /orders/123/fulfillments/1")
...
### POST /orders/{order\_id}/fulfillments[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-ordersorder_idfulfillments "Direct link to POST /orders/{order_id}/fulfillments")
...
| happened\_at | Date when the Fulfillment Event occured was created in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). |
| estimated\_delivery\_at | Estimated date when the package will be delivered in [ISO 8601 format](http://en.wikipedia.org/wiki/ISO_8601). |
...
| returned\_to\_sender | Package was returned to sender. |
| lost | Package lost. |
| failure | Package delivery failed. |

#### POST /orders/123/fulfillments[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#post-orders123fulfillments "Direct link to POST /orders/123/fulfillments")
...
#### Any invalid request wil return a 402 code for example[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#any-invalid-request-wil-return-a-402-code-for-example "Direct link to Any invalid request wil return a 402 code for example")
...
### DELETE /orders/${order\_id}/fulfillments/{id}[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-ordersorder_idfulfillmentsid "Direct link to DELETE /orders/${order_id}/fulfillments/{id}")

Remove a Fulfillment Event

#### DELETE /orders/123/fulfillments/1[](https://tiendanube.github.io/api-documentation/resources/abandoned-checkout#delete-orders123fulfillments1 "Direct link to DELETE /orders/123/fulfillments/1")
